...
#ifdef _MSC_VER
	// See this: http://msdn.microsoft.com/en-us/library/windows/desktop/aa384242(v=vs.85).aspx (Rules for using pointers)
	/* Save memory size of buffer */
	ULONG dwBufLen = sizeof(AdapterInfo);

	// Initializing to stop memory leak checker complaining about uninitialized access in GetAdaptersInfo()
	memset(AdapterInfo, 0, sizeof(AdapterInfo));
	int size = sizeof(AdapterInfo);
#else
	/* Save memory size of buffer */
    ...
    uint32_t net_address)
{
    ...
    // broadcast_address = net_address | ~net_mask ;       
    // bip_set_broadcast_addr(broadcast_address);
    portParams->bipParams.broadcast_addr = net_address | ~net_mask;
#endif
}

/* on Windows, ifname is the dotted ip address of the interface */
void bip_set_interface(PORT_SUPPORT *portParams, char *ifname)
{
    struct in_addr address;

    /* setup local address */
    if (bip_get_addr( portParams ) == 0) {
        bip_set_addr(portParams, inet_addr(ifname));
    }
    if (BIP_Debug) {
        address.s_addr = bip_get_addr(portParams);
        fprintf(stderr, "Interface: %s\n", ifname);
    }
    /* setup local broadcast address */
    if (bip_get_broadcast_addr( portParams ) == 0) {
        address.s_addr = bip_get_addr(portParams );
        set_broadcast_address(portParams, address.s_addr);
    }
}
...
bool bip_init2(PORT_SUPPORT *portParams, char *ifname)
{
    ...
    if (address.s_addr == 0) {
        ...
        bip_set_addr(portParams, address.s_addr);
    }
    if (BIP_Debug) {
        fprintf(stderr, "IP Address: %s\n", inet_ntoa(address));
    }

	// todo - bip_get_broadcast_addr ALSO fetches the netmask, redundant, remove
    portParams->bipParams.netmask = getIpMaskForIpAddress(address.s_addr);
    ...
    if (BIP_Debug) {
        broadcast_address.s_addr = bip_get_broadcast_addr(portParams );
        fprintf(stderr, "IP Broadcast Address: %s\n",
            inet_ntoa(broadcast_address));
        fprintf(stderr, "UDP Port: 0x%04X [%hu]\n", ntohs(portParams->bipParams.nwoPort),
            ntohs(portParams->bipParams.nwoPort));
    }
    portParams->bipParams.broadcast_addr = bip_get_broadcast_addr(portParams);
    portParams->bipParams.local_addr = bip_get_addr(portParams);

    /* assumes that the driver has already been initialized */
    portParams->bipParams.socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    // bip_set_socket(portParams->bip_params.socket);
    ...
    rv = setsockopt(portParams->bipParams.socket, SOL_SOCKET, SO_REUSEADDR, (char *)&value,
        sizeof(value));
    if (rv < 0) {
    ...
    rv = setsockopt(portParams->bipParams.socket, SOL_SOCKET, SO_BROADCAST, (char *)&value,
        sizeof(value));
    if (rv < 0) {
    ...
    rv = bind(portParams->bipParams.socket, (const struct sockaddr *) &sin,
        sizeof(struct sockaddr));
    if (rv < 0) {
        fprintf(stderr, "bip: failed to bind to %s port %hu\n",
            inet_ntoa(sin.sin_addr), ntohs(portParams->bipParams.nwoPort));
        close(portParams->bipParams.socket);
        //bip_set_socket(-1);
        return false;
    }
