    ...
    uint16_t mtu_len)
{
    ...
    bvlc_dest.sin_family = 2; // todonext4  AF_INET;
    bvlc_dest.sin_addr.s_addr = dest->sin_addr.s_addr;
    bvlc_dest.sin_port = dest->sin_port;
    memset(&(bvlc_dest.sin_zero), '\0', 8);
    /* Send the packet */
    // todonext4 - resolve the diff between this sendto and the bip.c sendto...?
    return sendto(portParams->bipParams.socket, (char *)mtu, mtu_len, 0,
...
//                && (bip_dest.sin_port == portParams->bip_params.nwoPort)) {
//                continue;
//            }
...
{       /* number of bytes of data */
    ...
    if ((dest->net == BACNET_BROADCAST_NETWORK) || ((dest->net > 0) &&
        ...
        if (Remote_BBMD.sin_port) {
            ...
            address.s_addr = portParams->bipParams.broadcast_addr ;
...
{
    BACNET_BVLC_RESULT result_code = BVLC_RESULT_SUCCESSFUL_COMPLETION ;   /* aka, BVLC_RESULT_SUCCESSFUL_COMPLETION */
    ...
    switch (BVLC_Function_Code) {
        case BVLC_RESULT:
            if (received_bytes >= 6) {
				...
				(void)decode_unsigned16(&npdu[4], (uint16_t*) &result_code);
                BVLC_Result_Code = (BACNET_BVLC_RESULT) result_code;
               //  debug_printf("BVLC: Result Code=%d\n", BVLC_Result_Code);
